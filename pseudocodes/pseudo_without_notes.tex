...
\begin{algorithm}[H] % [H]固定位置
	...
	\begin{algorithmic}[1]
		\STATE 初始化当前网络参数 $\theta$ 和目标网络参数 $\bar{\theta} \leftarrow \theta$
		\STATE 初始化经验回放$D$
		\FOR {回合数 $m = 1,2,\cdots,M$}
			...
			\FOR {时步 $t= 1,2,\cdots,T$}
				...
				\STATE 计算$Q$的期望值，即$y_{i}=r_t+\gamma\max _{a_{i+1}}Q_{\bar{\theta}}(s_{i+1},a)$
				\STATE 计算损失 $L(\theta)=\left(y_{i}-Q_{\theta}\left(s_{i}, a_{i}\right)\right)^{2}$ 并梯度下降更新参数 $\theta$
				\STATE 每$C$步复制参数到目标网络 $\bar{\theta} \leftarrow \theta$
			\ENDFOR
		\ENDFOR
	\end{algorithmic}
\end{algorithm}
