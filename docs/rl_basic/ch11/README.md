...
$\text{DDPG}$ 算法虽然在连续动作空间中表现不错，但有个很大的缺点，那就是它容易出现 $Q$ 值的过估计问题（ $\text{overestimation}$ ），这会导致策略学习的不稳定，甚至发散。为了解决这个问题，$\text{TD3}$ 算法（$\text{Twin Delayed DDPG}$ ）被提出，提出了三种关键的改进方法来缓解 $Q$ 值过估计的问题，一是 双 $Q$ 网络，二是 延迟更新，三是目标策略平滑。
